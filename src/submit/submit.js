...
import { formVariable } from "./constant.js";

export function submitSuccess(e, form) {
  console.log("Inside Submit Success.");
  const { payload } = e;
  // const redirectUrl = form.dataset.redirectUrl || payload?.body?.redirectUrl;
  const isRedirectUrl = formVariable.isRedirectUrl;
  const redirectUrl = formVariable.redirectUrl;
  // const thankYouMsg =
  //   form.dataset.thankYouMsg || payload?.body?.thankYouMessage;

  const thankYouMsg = formVariable.thankYouMessage;
...
export function submitFailure(e, form) {
  console.error("Inside Submit Failure.");
  let errorMessage = form.querySelector(".form-message.error-message");
  if (!errorMessage) {
    errorMessage = document.createElement("div");
    errorMessage.className = "form-message error-message";
  }
  errorMessage.innerHTML = formVariable.errorMessage;
...
async function submitDocBasedForm(form, captcha) {
  console.log("Submitting Document based form.");
  try {
...
export async function handleSubmit(e, form, captcha) {
  ...
  if (valid) {
    console.log("Inside Valid form submission.");
    e.submitter?.setAttribute("disabled", "");
    if (form.getAttribute("data-submitting") !== "true") {
      console.log("Form is not being submitted.");
      form.setAttribute("data-submitting", "true");
      form
        .querySelectorAll(".form-message.show")
        .forEach((el) => el.classList.remove("show"));
      console.log("Data Set Source:", form.dataset.source);
      if (form.dataset.source === "sheet") {
        console.log("Inside Sheet based form submission.");
        await submitDocBasedForm(form, captcha);
      }
    }
  } else {
    console.log("Inside Document based form submission.");
    const firstInvalidEl = form.querySelector(":invalid:not(fieldset)");
    if (firstInvalidEl) {
      console.log("Focusing on first invalid element.");
