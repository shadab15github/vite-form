...
import { handleSubmit } from "./submit-offline-wrapper.js";
...
const constraintsDef = Object.entries({
  ...
  "number|range|date": [["maximum", "Max"], ["max", "min"], "step"],
...
function createRadioOrCheckboxGroup(fd) {
  ...
  fd.enum.forEach((value, index) => {
    const label =
      typeof fd.enumNames?.[index] === "object" && fd.enumNames?.[index] != null
...
function createImage(fd) {
  ...
  const image = `
        <picture>
            <source srcset="${imagePath}?width=200&optimize=medium" media="(min-width: 600px)">
...
export async function fetchForm(pathname) {
  ...
  if (path.startsWith(window.location.origin) && !path.endsWith(".json")) {
    ...
    path += "/jcr:content/root/section/form.json";
  }
