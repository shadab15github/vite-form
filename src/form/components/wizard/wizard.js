...
export class WizardLayout {
  ...
  // }
  getSteps(panel) {
    // A step = an immediate child ".panel-wrapper"
    return [...panel.querySelectorAll(":scope > .panel-wrapper")];
  }
  ...
  //   ) {
  //     if (
  //       sibling.dataset.visible !== "false" &&
  //       sibling.tagName === "FIELDSET"
  //     ) {
  //       return sibling;
  //     }
  //   }
  ...
  getEligibleSibling(current, forward = true) {
    // Move between step wrappers only
    const steps = this.getSteps(current.parentElement);
    const i = steps.indexOf(current);
    if (i === -1) return null;
    const dir = forward ? 1 : -1;
    for (let j = i + dir; j >= 0 && j < steps.length; j += dir) {
      if (steps[j].dataset.visible !== "false") return steps[j];
    }
    return current;
  }
  /**
  ...
  validateContainer(container) {
    ...
    const isValid = fieldElements.reduce((valid, fieldElement) => {
      const isHidden =
        fieldElement.closest(".field-wrapper")?.dataset?.visible === "false";
  ...
  navigate(panel, forward = true) {
    ...
    const navigateTo = valid
      ? this.getEligibleSibling(current, forward)
      : current;
    ...
    if (navigateTo && current !== navigateTo) {
      ...
      const navigateToMenuItem = panel.querySelector(
  ...
  static handleMutation(panel, mutationsList) {
    mutationsList.forEach((mutation) => {
      ...
      if (type === "attributes" && attributeName === "data-visible") {
        ...
        // for active panel
        // panel
        //   .querySelector(".current-wizard-step")
        //   ?.classList.remove("current-wizard-step");
        // const activePanel = panel.querySelector(`#${target?.id}`);
        // activePanel?.classList.add("current-wizard-step");
        // Flip active step when a panel-wrapper gets data-active
        panel
          .querySelector(".current-wizard-step")
          ?.classList.remove("current-wizard-step");
        const activePanel = target?.classList?.contains("panel-wrapper")
          ? target
          : null;
        activePanel?.classList.add("current-wizard-step");
        // for active menu item
        panel
          .querySelector(".wizard-menu-active-item")
          ?.classList.remove("wizard-menu-active-item");
        menuItems
          .querySelector(`[data-index="${activePanel.dataset.index}"]`)
          ?.classList.add("wizard-menu-active-item");
  ...
  applyLayout(panel) {
    ...
    panel
      .querySelector(":scope > .panel-wrapper")
      ?.classList.add("current-wizard-step");
