...
import initializeRuleEngineWorker from "./wroker.js";
...
async function fieldChanged(payload, form, generateFormRendition) {
  ...
  changes.forEach((change) => {
    ...
    switch (propertyName) {
      ...
      case "value":
        if (
          ["number", "date", "text", "email"].includes(field.type) &&
          (displayFormat || displayValueExpression)
        ) {
          ...
          field.querySelectorAll(`input[name=${name}]`).forEach((el) => {
            const exists =
              (Array.isArray(currentValue) &&
                currentValue.some((x) =>
                  compare(x, el.value, type.replace("[]", ""))
                )) ||
...
function formChanged(payload, form) {
  ...
  changes.forEach((change) => {
    ...
    switch (propertyName) {
      case "activeChild":
        handleActiveChild(currentValue?.id, form);
        break;
      default:
        break;
    }
...
function applyRuleEngine(htmlForm, form, captcha) {
  htmlForm.addEventListener("change", (e) => {
    ...
    if (
      field.type === "checkbox" &&
      field.dataset.fieldType === "checkbox-group"
    ) {
      ...
      field.type === "radio" &&
      field.dataset.fieldType === "radio-group"
    ) {
      const el = form.getElement(id);
      el.value = value;
    } else if (field.type === "checkbox") {
      form.getElement(id).value = checked
        ? value
        : field.dataset.uncheckedValue;
    } else if (field.type === "file") {
    ...
    const observer = new MutationObserver((mutationsList) => {
      mutationsList?.forEach((mutation) => {
        if (
          ...
          mutation.attributeName === "data-field-model-changed"
        ) {
          callback(fieldDiv, formModels[fieldDiv.closest("form")?.dataset?.id]);
        }
      });
    });
